...
## Guidelines to structure your WDL Workflow
> Note: Give a graphical representation of the WDL workflow structure you would expect users should see
- Structs will be at the top of the WDL script, followed by the workflow itself, followed by all of its corresponding tasks.
    > Note: While the WDL workflow allows users to place structs anywhere in the workflow, to promote consistency we require that structs be placed right on top for easier readability. 
- Tasks should be broken down into as small of operations as possible.
    - If a task uses more than two tools, it should probably be broken up into individual tasks.
...
- In general, runtime attributes should be defined whenever possible in order to enable execution on as many backends as possible.
    > Note: Some runtime attributes will be ignored based on the backend/WDL engine being used to run your workflow. Refer to the WDL 101 guide for more details. 

## Stylistic Guidelines for WDL Workflows
...
- As with all repositories, each workflow should include a detailed README containing:
    ...
    - Basic diagram illustration of the flow of data
